...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		Color tetscolor  = new Color(100, 40, 100);
		Color tetscolor1 = new Color(200, 20, 40);


		//System.err.println(blend(tetscolor,tetscolor1,0.25));
	...
	public static Color[][] read(String fileName) {
		...
		for (int i = 0; i < numRows; i++){
			for (int j = 0; j < numCols; j++){
				...
				pixGreen = in.readInt();
				pixBlue = in.readInt();
				image[i][j] = new Color (pixRed,pixGreen,pixBlue);
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		//// Replace the following statement with your code
		Color[][] hFlippedImage = new Color[image.length][image[0].length];
		for (int i = 0; i < image.length; i++){
			for (int j = 0; j < image[0].length; j++){
				hFlippedImage[i][image[0].length - 1 - j] = image[i][j];
			}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		Color[][] vFlippedImage = new Color[image.length][image[0].length];
		for (int i = 0; i < image.length; i++){
			for (int j = 0; j < image[0].length; j++){
				vFlippedImage[image.length - 1 - i][j] = image[i][j];
			}
		}
	...
	public static Color luminance(Color pixel) {
		//// Replace the following statement with your code
		int greyColor = (int) (pixel.getRed() * 0.299 + pixel.getGreen() * 0.587 + pixel.getBlue() * 0.114);
		Color greyScaledPixel = new Color (greyColor, greyColor, greyColor);
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double heightFactor = image.length/(height + 0.0);
		double widthFactor = image[0].length/(width + 0.0);
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		morphedImage = scaled(target, source[0].length, source.length);
		/*Runigram.display(source);
		StdDraw.pause(1000);
		Runigram.setCanvas(target);
		Runigram.display(target);
		StdDraw.pause(1000); 
		Runigram.setCanvas(scaledTarget); 
		Runigram.display(scaledTarget);
		StdDraw.pause(1000);*/
		for (int i = n; i >= 0; i--){
			morphedImage = blend(source, scaledTarget, (i/(n + 0.0)));
